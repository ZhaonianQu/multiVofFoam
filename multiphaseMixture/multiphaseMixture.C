...
Foam::multiphaseMixture::wrmean() const
{
    auto iter = phases_.cbegin();

    tmp<volVectorField> twr = 0*iter().wr();
    volVectorField& wr = twr.ref();

    for (++iter; iter != phases_.cend(); ++iter)
    {
        if (iter().name() == "water"||iter().name() == "air" ) continue;	
    	tmp<volScalarField> cd = iter()*iter().rho()/rho();
        wr += iter().wr()*cd();
    }
...
Foam::multiphaseMixture::wr2mean() const
{
    auto iter = phases_.cbegin();

    tmp<volTensorField> twr2 = 0*iter()*(iter().wrVector()*iter().wrVector());
    volTensorField& wr2 = twr2.ref();

    for (++iter; iter != phases_.cend(); ++iter)
    {
        if (iter().name() == "water"||iter().name() == "air" ) continue;	
        tmp<volScalarField> cd = iter()*iter().rho()/rho();
        wr2 += cd() *(iter().wrVector()*iter().wrVector());
    }
...
)
{
    ...
    for (phase& alpha : phases_)
    {
	    ...
	    surfaceScalarField phiwrDim_
		(
			 "phiwrDim_",
			 fvc::flux(alpha.wr()-wrmean())//minus wr_mean here: -mixture.wrmean()
		);
        ...
        for (phase& alpha2 : phases_)
        {
            ...
            /* Different oil droplet size would not compress interface 
            because in a coarse mesh, they are one phase actually. */
            if (!(alpha.name() == "water" || 
            	alpha2.name() == "water"  || 
            	alpha.name() == "air"     || 
            	alpha2.name() == "air")) continue; //check .name() returns water or alpha.water
        ...
        //add the diffusion flux
        if (!(alpha.name() == "water" || alpha.name() == "air"  ))
        {
        	alphaPhiCorr -= fvc::interpolate(D_) * fvc::snGrad(alpha) * mesh_.magSf();
		}
